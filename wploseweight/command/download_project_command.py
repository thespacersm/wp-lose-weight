...
class DownloadProjectCommand(AbstractCommand):
    command_name = "project:download"
    ...
    def run(self, ftp_ip: str, ftp_username: str, ftp_password: str, project_name: str):
        ...
        local_uploads_dir = self.uploads_optimization_service.download_project(
            options,
            progress_callback=notify,
        )
        click.echo(f"Download completato in: {local_uploads_dir}")
